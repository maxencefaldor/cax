"""Lenia update module."""

from collections.abc import Callable

import jax
import jax.numpy as jnp
from cax.core.update.update import Update
from cax.models.lenia.lenia_perceive import bell
from cax.types import Input, Perception, State
from flax import nnx
from jax import Array

from .types import GrowthParams, RuleParams


def growth_exponential(u: Array, growth_params: GrowthParams) -> Array:
	"""Growth mapping function introduced in [1]."""
	return 2 * bell(u, growth_params.mean, growth_params.std) - 1


class LeniaUpdate(Update):
	"""Lenia update class with optional Flow Lenia support."""

	def __init__(
		self,
		T: int,
		state_scale: float,
		rules_params: RuleParams,
		growth_fn: Callable = growth_exponential,
		flow: bool = False,
		theta_A: float = 1.0,
		n: int = 2,
		dt: float = 0.2,
		dd: int = 5,
		sigma: float = 0.65,
		border: str = "torus",
	):
		"""Initialize the LeniaUpdate.

		Args:
			T: Time resolution.
			state_scale: Integer to scale the state.
			rules_params: Parameters for the rules.
			growth_fn: Growth function.
			flow: Whether to use Flow Lenia (True) or vanilla Lenia (False).
			theta_A: Threshold for alpha computation in Flow Lenia.
			n: Exponent for alpha computation in Flow Lenia.
			dt: Time step for flow displacement.
			dd: Maximum displacement distance.
			sigma: Spread parameter for reintegration tracking.
			border: Boundary condition ('torus' or 'wall').

		"""
		super().__init__()
		self.T = T
		self.state_scale = state_scale
		self.rules_params = jax.tree.map(nnx.Variable, rules_params)
		self.growth_fn = growth_fn
		self.flow = flow
		self.theta_A = theta_A
		self.n = n
		self.dt = dt
		self.dd = dd
		self.sigma = sigma
		self.border = border

		# Placeholder for num_channels; should be derived from rules_params
		num_channels = 3  # TODO: Replace with actual number of channels
		self.reshape_kernel_to_channel = nnx.Param(
			jax.vmap(lambda x: jax.nn.one_hot(x, num_classes=num_channels))(
				rules_params.channel_target
			)
		)

	def __call__(self, state: State, perception: Perception, input: Input | None = None) -> State:
		"""Apply the Lenia update rule with optional flow.

		Args:
			state: Current state of shape (y, x, c).
			perception: Perceived state of shape (y, x, k).
			input: External input (unused).

		Returns:
			Updated state of shape (y, x, c).

		"""
		# Compute growth per kernel
		g_k = self.rules_params.weight[None, None, ...] * self.growth_fn(
			perception, self.rules_params.growth_params
		)  # (y, x, k,)

		# Aggregate growth to channels
		g = jnp.dot(g_k, self.reshape_kernel_to_channel.value)  # (y, x, c,)

		if self.flow:
			# Flow Lenia update
			nabla_U = sobel(g)  # (y, x, 2, c)
			nabla_A = sobel(state.sum(axis=-1, keepdims=True))  # (y, x, 2, 1)
			alpha = jnp.clip(
				(state[:, :, None, :] / self.theta_A) ** self.n, 0.0, 1.0
			)  # (y, x, 1, c)
			F = nabla_U * (1 - alpha) - nabla_A * alpha  # (y, x, 2, c)
			state = apply_reintegration_tracking(
				state, F, self.dt, self.dd, self.sigma, self.border
			)
		else:
			# Vanilla Lenia update
			state = jnp.clip(state + 1 / self.T * g, 0.0, 1.0)

		return state


from jax.scipy.signal import convolve2d


def get_sobel_kernels():
	"""Define Sobel kernels exactly as in the reference Flow Lenia code."""
	kx = jnp.array([[1.0, 0.0, -1.0], [2.0, 0.0, -2.0], [1.0, 0.0, -1.0]], dtype=jnp.float32)
	ky = jnp.transpose(kx)  # [[1., 2., 1.], [0., 0., 0.], [-1., -2., -1.]]
	return kx, ky


def sobel_x(A, kx):
	"""Compute horizontal Sobel filter per channel."""
	# Apply convolve2d to each channel using vmap
	return jax.vmap(lambda a: convolve2d(a, kx, mode="same"), in_axes=2, out_axes=2)(A)


def sobel_y(A, ky):
	"""Compute vertical Sobel filter per channel."""
	return jax.vmap(lambda a: convolve2d(a, ky, mode="same"), in_axes=2, out_axes=2)(A)


def sobel(A):
	"""Compute gradients using Sobel filters, matching the reference Flow Lenia implementation.

	Args:
	    A: Input array of shape (y, x, c), where c is the number of channels.

	Returns:
	    Gradients of shape (y, x, 2, c), where axis -3 is [sobel_y(A), sobel_x(A)],
	    matching the reference code's [-dy, -dx] convention.

	"""
	kx, ky = get_sobel_kernels()

	# Compute gradients per channel
	grad_y = sobel_y(A, ky)  # -dy
	grad_x = sobel_x(A, kx)  # -dx

	# Stack gradients as [grad_y, grad_x] = [-dy, -dx]
	return jnp.stack([grad_y, grad_x], axis=2)  # (y, x, 2, c)


def apply_reintegration_tracking(state, F, dt, dd, sigma, border):
	"""Apply reintegration tracking to update the state based on the flow field.

	Args:
		state: Current state of shape (y, x, c).
		F: Flow field of shape (y, x, 2, c).
		dt: Time step for flow displacement.
		dd: Maximum displacement distance.
		sigma: Spread parameter for area computation.
		border: Boundary condition ('torus' or 'wall').

	Returns:
		New state of shape (y, x, c).

	"""
	SY, SX, C = state.shape

	# Generate all possible displacements
	dys = jnp.arange(-dd, dd + 1)
	dxs = jnp.arange(-dd, dd + 1)
	dys, dxs = jnp.meshgrid(dys, dxs, indexing="ij")
	dys = dys.flatten()
	dxs = dxs.flatten()

	# Compute grid positions
	y, x = jnp.arange(SY), jnp.arange(SX)
	Y, X = jnp.meshgrid(y, x, indexing="ij")
	pos = jnp.stack([Y, X], axis=-1) + 0.5  # (SY, SX, 2)

	# Compute target positions (mu)
	ma = dd - sigma  # Maximum allowed displacement
	F_clipped = jnp.clip(dt * F, -ma, ma)  # (SY, SX, 2, C)
	mu = pos[..., None] + F_clipped  # (SY, SX, 2, C)

	if border == "wall":
		mu = mu.at[:, :, 0, :].set(jnp.clip(mu[:, :, 0, :], sigma, SY - sigma))
		mu = mu.at[:, :, 1, :].set(jnp.clip(mu[:, :, 1, :], sigma, SX - sigma))

	# Define step function for each displacement
	def step(dy, dx):
		Xr = jnp.roll(state, (dy, dx), axis=(0, 1))  # (SY, SX, C)
		mur = jnp.roll(mu, (dy, dx), axis=(0, 1))  # (SY, SX, 2, C)

		if border == "torus":
			shifts_y = [-SY, 0, SY]
			shifts_x = [-SX, 0, SX]
			dpmu = jnp.min(
				jnp.stack(
					[
						jnp.abs(pos[..., None] - (mur + jnp.array([di, dj])[None, None, :, None]))
						for di in shifts_y
						for dj in shifts_x
					],
					axis=0,
				),
				axis=0,
			)  # (SY, SX, 2, C)
		else:
			dpmu = jnp.abs(pos[..., None] - mur)  # (SY, SX, 2, C)

		sz = 0.5 - dpmu + sigma  # (SY, SX, 2, C)
		clipped_sz = jnp.clip(sz, 0, min(1, 2 * sigma))  # (SY, SX, 2, C)
		area = jnp.prod(clipped_sz, axis=2) / (4 * sigma**2)  # (SY, SX, C)
		nX = Xr * area  # (SY, SX, C)
		return nX

	# Apply step function over all displacements
	nX = jax.vmap(step, in_axes=(0, 0))(dys, dxs)  # (num_displacements, SY, SX, C)
	new_state = jnp.sum(nX, axis=0)  # (SY, SX, C)

	return new_state
