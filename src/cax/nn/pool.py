...
class Pool(nnx.Pytree):
    ...
    The pool holds a PyTree of arrays whose first dimension is the pool size. It can be
    created from a PyTree with leading batch dimension. Sampling returns indices and the
    sliced batch for the same indices across all leaves.

    Attributes:
        size: Number of items in the pool (inferred from the leading dimension of the
            data).
    ...
    def create(cls, data: Any) -> Self:
        ...
        Args:
            data: PyTree whose leaves are arrays with shape `(N, ...)`, where `N` is the
                pool size.
    ...
    def sample(
        self, key: Array, *, batch_size: int, replace: bool = True
    ) -> tuple[Array, Any]:
        ...
        Returns:
            A tuple `(idxs, batch)` where `idxs` has shape `(batch_size,)` and `batch`
            is a PyTree with each leaf shaped `(batch_size, ...)`.

        """
        if not replace and batch_size > self.size:
            raise ValueError(
                f"batch_size ({batch_size}) must not exceed pool size ({self.size})"
            )
