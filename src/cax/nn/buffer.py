...
class Buffer(nnx.Pytree):
    ...
    The buffer stores a PyTree of arrays with a fixed capacity along the leading
    dimension. New batches are written sequentially with wrap-around semantics. Sampling
    draws indices from the subset of entries that have been written at least once.

    Attributes:
        size: Maximum number of items stored.
        data: PyTree of arrays with leading dimension `size`.
        is_full: Boolean mask of shape `(size,)` indicating which entries are
            initialized.
    ...
    def add(self, batch: Any) -> Self:
        ...
        Args:
            batch: PyTree whose leaves have shape `(B, ...)`, where `B` is the batch
                size.
        ...
        self.data = jax.tree.map(
            lambda data, batch: data.at[idxs].set(batch), self.data, batch
        )
    ...
    def sample(self, key: Array, *, batch_size: int) -> Any:
        ...
        are intended; the buffer is read-only under sampling, so duplicates are
        harmless.
        ...
        Returns:
            A PyTree with each leaf shaped `(batch_size, ...)`, sampled from filled
            slots.
