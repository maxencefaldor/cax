...
class ParticleLeniaPerceive(Perceive[ParticleLeniaState, Array]):
    ...
    def __call__(self, state: ParticleLeniaState) -> Array:
        ...
        Computes the force on each particle by taking the negative gradient of the
        energy field with respect to particle position. Particles are driven toward
        regions of high growth and away from regions of high repulsion.
    ...
    def compute_fields(
        self, state: ParticleLeniaState, x: Array
        ...
        and repulsion field (overlap prevention) at position x given all particle
        positions.
        ...
        """
        r = jnp.sqrt(
            jnp.clip(jnp.sum(jnp.square(x - state.position), axis=-1), min=1e-10)
        )
    ...
    def energy_field(self, state: ParticleLeniaState, x: Array) -> Array:
        ...
        Returns:
            Scalar energy value at position x, computed as repulsion minus growth
            (R - G).
