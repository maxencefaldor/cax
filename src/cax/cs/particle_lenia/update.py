...
class ParticleLeniaUpdate(Update[ParticleLeniaState, Array, Array]):
    ...
    Updates particle positions by integrating forces derived from energy field
    gradients. The temporal resolution T controls the step size, with higher T producing
    smaller, smoother position updates.
    """
    ...
    def __call__(
        ...
        Integrates forces to update particle positions with temporal resolution T.
        Particles move according to the gradient of the energy field, scaled by 1/T for
        smooth dynamics.

        Args:
            state: ParticleLeniaState containing current particle positions.
            perception: Array with shape (num_particles, num_spatial_dims) containing
                force vectors from the perception step.
