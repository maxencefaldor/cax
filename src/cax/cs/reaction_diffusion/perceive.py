...
This module implements the perception function for the Gray-Scott reaction-diffusion
system. It computes the discrete Laplacian of each chemical species using a fixed
convolution stencil, which represents the spatial diffusion component of the PDE.
"""
...
class ReactionDiffusionPerceive(Perceive[Array, Array]):
    ...
    def __init__(
        ...
        *,
        ...
        self.pad_mode = {"CIRCULAR": "wrap", "ZERO": "constant", "EDGE": "edge"}[
            padding
        ]
        self.channel_size = 2
    ...
    def __call__(self, state: Array) -> Array:
        ...
        Returns:
            Array with shape `(..., *spatial_dims, 4)` containing
            [U, lap(U), V, lap(V)].
        ...
        # Pad the spatial axes per the boundary condition, then convolve without
        # padding.
        ...
        padded = padded.reshape(
            -1, *[dim + 2 for dim in spatial_dims], self.channel_size
        )
