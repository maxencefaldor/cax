...
Laplacian perception) with the cubic reaction term and feed/kill dynamics for two
chemical species U and V.
"""
...
class ReactionDiffusionUpdate(Update[Array, Array, Array]):
    ...
    def __call__(
        ...
        dv = (
            self.diffusion_rate_v * lap_v
            + reaction
            - (self.feed_rate + self.kill_rate) * v
        )
