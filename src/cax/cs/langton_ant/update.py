...
This module implements the update rule for Langton's Ant and its multi-color
generalizations. The ant turns based on the current cell color, flips the cell to the
next color, and moves forward one step.
"""
...
class LangtonAntUpdate(Update[LangtonAntState, Array, Array]):
    ...
    Applies the generalized Langton's Ant rule: the ant turns according to the current
    cell color, flips the cell to the next color in the cycle, and advances one step in
    its new heading direction. Boundaries wrap periodically.
    """
    ...
    def __call__(
        ...
        new_position = jnp.stack(
            [(row + delta[0]) % grid_height, (col + delta[1]) % grid_width]
        )

        return replace(
            state, grid=new_grid, position=new_position, direction=new_direction
        )
