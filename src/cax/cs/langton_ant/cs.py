...
This module implements Langton's Ant and its multi-color generalizations. Langton's Ant
is a two-dimensional cellular automaton with a mobile agent (the "ant") that moves on a
grid, turning and flipping cell colors according to a rule string. Despite its simple
definition, the system exhibits complex emergent behavior — notably the formation of a
"highway" after approximately 10,000 steps in the classic RL variant.
"""
...
class LangtonAnt(ComplexSystem[LangtonAntState, Array]):
    ...
    A two-dimensional cellular automaton with a mobile agent that traverses a grid of
    colored cells. At each step the ant turns according to the color of its current
    cell, advances the cell to the next color in a cyclic sequence, and moves forward
    one step. The rule is specified as a sequence of turn directions — one per color —
    encoded as a string over the alphabet {R, L, N, U}.
    ...
    @override
    def _step(
        self, state: LangtonAntState, input: Array | None = None
    ) -> LangtonAntState:
    ...
    def turns_from_rule_string(cls, rule_string: str) -> Array:
        ...
        Returns:
            Array of shape (num_colors,) with integer turn values where 0=no turn,
                1=right (90 degrees clockwise), 2=u-turn (180 degrees), 3=left (90
                degrees counter-clockwise).

        """
        if len(rule_string) < 2:
            raise ValueError(
                f"rule_string must have at least 2 characters, got {rule_string!r}"
            )
        ...
        turns = jnp.array(
            [TURN_CHAR_TO_INT[char] for char in rule_string], dtype=jnp.int32
        )
        return turns
    ...
    def render(self, state: LangtonAntState) -> Array:
        ...
        Visualizes the grid by mapping each cell color to a distinct hue. For 2-color
        rules the rendering is grayscale (black for color 0, white for color 1). For
        multi-color rules, color 0 (unvisited) is black and colors 1 through
        num_colors-1 are evenly spaced around the HSV hue wheel at full saturation and
        value.
