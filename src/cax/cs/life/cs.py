...
class Life(ComplexSystem[Array, Array]):
    ...
    def birth_survival_from_string(cls, rule_golly: str) -> tuple[Array, Array]:
        ...
        if not all(0 <= num <= 8 for num in birth_numbers + survival_numbers):
            raise ValueError(
                f"Numbers in rule string must be between 0 and 8, got {rule_golly!r}"
            )

        # Create birth and survival rules
        birth = jnp.array(
            [
                1.0 if num_neighbors in birth_numbers else 0.0
                for num_neighbors in range(9)
            ]
        )
        survival = jnp.array(
            [
                1.0 if num_neighbors in survival_numbers else 0.0
                for num_neighbors in range(9)
            ]
        )
    ...
    def render(self, state: Array) -> Array:
        ...
        Converts the Life state to an RGB visualization by replicating the
        single-channel state values across all three color channels, resulting in a
        grayscale image where alive cells appear white and dead cells appear black.
