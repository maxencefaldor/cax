...
This module implements the update rule for Elementary Cellular Automata based on Wolfram
codes. Each cell's next state is determined by converting its three-cell neighborhood to
a 3-bit index and looking up the result in the Wolfram code table.
...
class ElementaryUpdate(Update[Array, Array, Array]):
    ...
    def __call__(
        ...
        Converts each cell's three-cell neighborhood to a 3-bit index (left*4 + self*2 +
        right) and looks up the corresponding output in the Wolfram rule table.

        Args:
            state: Current state (unused, next state computed solely from perception).
            perception: Array with shape (..., width, 3) containing the three-cell
                neighborhood in [self, right, left] order.
        ...
        """
        index = (
            perception[..., 2:3] * 4 + perception[..., 0:1] * 2 + perception[..., 1:2]
        ).astype(jnp.int32)
