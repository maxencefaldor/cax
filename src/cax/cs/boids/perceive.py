...
class BoidsPerceive(Perceive[BoidsState, BoidsPerception]):
    """Boids perception.

    Computes steering accelerations for each boid by applying the boid policy. The
    policy evaluates each boid's local neighborhood and determines the desired steering
    direction to achieve flocking behaviors.
    """
    ...
    def __call__(self, state: BoidsState) -> BoidsPerception:
        ...
        which evaluates neighbors and determines the desired steering based on
        ...
        Returns:
            BoidsPerception containing acceleration array with shape
            (num_boids, num_spatial_dims) representing the steering forces for each
            boid.
