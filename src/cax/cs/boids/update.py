...
This module implements the update rule for Boids, which integrates steering
accelerations to update velocities and positions. Includes optional velocity damping
through friction and periodic boundary conditions.
...
class BoidsUpdate(Update[BoidsState, BoidsPerception, Array]):
    ...
    def __call__(
        ...
        Integrates steering accelerations to update velocities with friction, then
        updates positions. Applies periodic boundary conditions by wrapping positions to
        [0, 1].
