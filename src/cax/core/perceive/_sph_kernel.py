...
The four quantities a particle perceives --- density, the neighborhood-average state, the
state gradient and the density gradient --- are four sums over the *same* pairs. Written as
array operations each one walks an array with an entry per pair, so a few thousand
particles move gigabytes through memory to produce a few megabytes of answer. Here they
accumulate together, one tile of neighbors at a time, in registers, and only the answers
are ever written. It is the tiling attention kernels use, for the same reason.

The saving is in *space*, not in time complexity: every pair is still visited, but nothing
with an entry per pair is ever built. That is what lets a cloud grow past the point where
the array version simply runs out of memory.
...
automatic differentiation, so the backward pass is derived by hand below --- the part where
a mistake would train quietly rather than fail.
...
Nothing here is specialized to a particular cloud size. The tile shape is chosen from the
...
# How deep the compiler pipelines the loop over neighbor tiles: with more stages it starts
# fetching the next tile while the current one is still being reduced, which is what a loop
# of dependent global loads wants.
NUM_STAGES = 3

# The backward is a different kernel with a different appetite. It carries four accumulators
# instead of one and reads ten arrays instead of four, so it runs out of registers at a tile
# the forward is happy with, and it wants a smaller one. Attention kernels split these the
# same way, tuning the forward and the two backward passes separately.
BACKWARD_BLOCK_QUERIES = 32


def _num_warps(channel_size: int) -> int:
	"""Choose the warp count from the channel width, which is what sets the tile's area."""
...
def _weights(dx: Array, dy: Array, support_radius: float) -> tuple[Array, Array]:
	...
	weight = jnp.where(inside > 0.0, (4.0 / (math.pi * support_radius**8)) * inside**3, 0.0)
...
def _moments_kernel(
	...
	*,
	...
	"""Accumulate the average, the state gradient and the density gradient in one pass."""
	queries = _queries(block_queries)
	own_x, own_y = x_ref[queries][:, None], y_ref[queries][:, None]
	...
	average, first_x, first_y, weighted_x, weighted_y, raw_x, raw_y = jax.lax.fori_loop(
		0, num_tiles, tile, (channels, channels, channels, scalars, scalars, scalars, scalars)
	)
...
def _forward(
	position: Array, feature: Array, *, support_radius: float, mass: float, period: float | None
	...
	# The tiling is static, so a cloud the tiles do not divide is padded out and the extra
	# entries are masked away inside the kernel. Nothing is specialized to one size.
	...
	triton = plt.CompilerParams(num_warps=_num_warps(channel_size), num_stages=NUM_STAGES)
	...
	# complete before the rest can start. Hence two passes, not one. Padded particles are
	# given no volume at all, so they cannot weigh on anything even before the mask.
	...
	average, gradient_x, gradient_y, density_gradient_x, density_gradient_y = pl.pallas_call(
		functools.partial(_moments_kernel, channel_size=channel_size, **shape()),
		grid=grid,
		compiler_params=triton,
		in_specs=[whole_spec] * 4,
		out_specs=[channel_spec, channel_spec, channel_spec, query_spec, query_spec],
		out_shape=[
...
def _terms(rx: Array, ry: Array, radius: float) -> tuple[Array, Array, Array, Array, Array]:
	...
	spiky_derivative = jnp.where(
		spiky_live,
		(30.0 / (math.pi * radius**5)) * margin * (margin + 2.0 * distance) / (distance * distance),
		0.0,
	)
...
def _vjp_kernel(
	...
	*,
	...
	Where the chain rule asks for the derivative with respect to a *neighbor*, antisymmetry
	of the pair kernels turns what would be a scatter into a second gather --- so this has
	the same shape as the forward and needs no atomics. Each array is read twice, once for
	the particles this block answers for and once per tile of their neighbors; the suffixes
	say which.
	"""
	queries = _queries(block_queries)
	own_x, own_y = x_ref[queries][:, None], y_ref[queries][:, None]
	...
	def tile(k, carry):
		...
		gradient_x, gradient_y = cotangent_gradient_x_ref[span], cotangent_gradient_y_ref[span]
		...
		weight, poly_scale, spiky_scale, spiky_derivative, distance = _terms(rx, ry, support_radius)
		...
		if wants_feature:
			...
			d_feature += own_volume * ((-spiky_x) @ gradient_x + (-spiky_y) @ gradient_y)
			...
			# It scales every neighbor's reading of it, so the cotangent gathers from the
			# neighbors that looked.
			...
			d_volume += jnp.sum(
				(-spiky_x) * gradient_x_onto_own + (-spiky_y) * gradient_y_onto_own, axis=-1
			)
			d_volume -= jnp.sum(
				(-spiky_x) * gradient_x_dot_feature + (-spiky_y) * gradient_y_dot_feature,
				axis=-1,
			)
		...
		if wants_position:
			...
			own_gradient_x_dot_own = jnp.sum(own_gradient_x * own_feature, axis=-1)[:, None]
			own_gradient_y_dot_own = jnp.sum(own_gradient_y * own_feature, axis=-1)[:, None]
			...
			# The neighbor's half of the vector kernel enters with the opposite sign: the
			# displacement flips too, and the two flips cancel for the poly6 route above
			# but not for this one.
			vector_x = forward_x - backward_x + mass * (own_density_gradient_x - density_gradient_x)
			vector_y = forward_y - backward_y + mass * (own_density_gradient_y - density_gradient_y)
...
@functools.partial(
	jax.jit,
	static_argnames=("support_radius", "mass", "period", "wants_position", "wants_feature"),
)
def _backward(
	position: Array,
	feature: Array,
	...
	*,
	...
	cotangent_density, cotangent_average, cotangent_gradient, cotangent_density_gradient = (
		cotangents
	)
	num_particles = position.shape[0]
	channel_size = feature.shape[-1]
...
def sph_moments(
	...
	"""
	return _forward(position, feature, support_radius=support_radius, mass=mass, period=period)


def _moments_fwd(position, feature, support_radius, mass, period):
	outputs = _forward(position, feature, support_radius=support_radius, mass=mass, period=period)
...
def _moments_bwd(support_radius, mass, period, residuals, cotangents):
	...
	complete before the positions can be resolved --- the same two-pass shape the forward
