...
- `"hexagonal"` --- a triangular lattice stored in a square array, whose axes are read as
  the basis vectors `(1, 0)` and `(1/2, sqrt(3)/2)` rather than as a Cartesian frame. Two
  opposite corners of the block are then not adjacent, leaving six neighbors that are all
  exactly one unit away. Two dimensions only.
"""
...
def _square_distance(offset: tuple[int, ...], neighborhood: Neighborhood) -> float:
	"""Measure a neighbor's squared distance from the center in the lattice's own geometry.

	On a triangular lattice the array axes are not perpendicular, so two cells a diagonal
	step apart can still be nearest neighbors. Measuring through the basis is what makes
	all six of them come out equidistant.
	"""
...
def _neighbor_offsets(num_dims: int, neighborhood: Neighborhood) -> list[tuple[int, ...]]:
	"""List the offsets of the neighbors a neighborhood includes, excluding the center."""
	...
	if neighborhood == "hexagonal" and num_dims != 2:
		raise ValueError(f"The hexagonal neighborhood is two-dimensional, got num_dims={num_dims}.")
...
def grad2_kernel(
	*, num_dims: int, neighborhood: Neighborhood = "von_neumann", normalize: bool = True
	...
	The neighborhood decides how isotropic the result is. `"von_neumann"` is the cheapest
	stencil, but its leading error favors the grid axes with four-fold symmetry --- a bias
	a system asked to behave the same in every direction will find and use. `"moore"`
	brings in the diagonals, which is the familiar
	`[[1, 2, 1], [2, -12, 2], [1, 2, 1]]` up to the scale `normalize` removes; on a quartic
	rotated by 45 degrees the von Neumann answer moves by 50% and this one by 20%.

	`"hexagonal"` removes the question. Its six neighbors are all one unit away, so they are
	weighted equally and no direction is singled out at all. Note that an array read as a
	triangular lattice also has to be *drawn* as one --- see `cax.utils.hex_to_square`.
