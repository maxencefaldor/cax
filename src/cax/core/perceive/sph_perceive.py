...
Where a convolution reads a fixed stencil of neighbors, particles have no stencil: who is
nearby changes every step. Smoothed particle hydrodynamics answers the same questions ---
what is the field here, which way is it changing --- as sums over whatever particles happen
to lie within a support radius, weighted by a kernel that falls smoothly to zero at the
edge. The smoothness is what keeps the whole thing differentiable as neighbors come and go.
"""
...
def poly6_kernel(square_distance: Array, support_radius: float) -> Array:
	...
	Poly6 takes the squared distance directly, so nothing has to take a square root to use
	it, and it falls to zero smoothly at the support radius.
...
def spiky_gradient_kernel(displacement: Array, distance: Array, support_radius: float) -> Array:
	...
	Args:
		displacement: Vectors between pairs of particles, shape `(..., num_spatial_dims)`.
	...
	scale = jnp.where(
		(inside > 0.0) & (distance > 1e-9), -normalization * inside**2 / safe_distance, 0.0
	)
...
def _log_normalize(vector: Array) -> Array:
	...
	derivative is unbounded at the origin, so differentiating through one there gives `nan`
	however small the epsilon underneath it --- and most particles sit in a neighborhood
	where nothing is happening yet, so the vector really is zero. Branching before the
	square root, not after, keeps the derivative finite: the constant the masked branch
	takes has no gradient at all.
	"""
...
class SPHPerceive(Perceive[Particles, Array]):
	...
	Four quantities are gathered for every particle: its own state, the neighborhood-average
	state, the gradient of the state, and the gradient of the particle density.

	The average stands in for a Laplacian. The difference between a particle's own state and
	its neighborhood average is proportional to the Laplacian for small radii, and the
	explicit smoothed particle hydrodynamics Laplacian is numerically ill-behaved, so the
	average is carried instead and the network can take the difference itself.

	The density gradient has no counterpart on a lattice, where cells are evenly spaced by
	construction. Particles crowd and spread, so it is the signal that says which way the
	cloud is getting denser.

	Every pair is evaluated. That is quadratic in the number of particles, and for the
	sizes these systems are trained at it is also the fastest thing to do: the neighborhood
	holds a few percent of the cloud, so a spatial index spends more on bookkeeping than it
	saves on arithmetic.
	"""
	...
	def __init__(
		...
		*,
		...
		Args:
			support_radius: Distance beyond which particles do not interact.
			mass: What each particle weighs. Density is a sum of masses, so this sets the
				scale of the density gradient --- and that quantity is handed to a network
				alongside states of order one. A cloud of `n` particles usually wants
				`1 / n`, which makes the whole cloud weigh one and leaves the density near
				one as well.
			log_normalize: Whether to compress the two gradients to `log(1 + norm)` while
				keeping their direction. A kernel gradient carries a factor of the support
				radius to a negative power, so where particles crowd it reaches thousands
				while the states beside it are of order one --- and a network fed both
				diverges. Compressing keeps the direction, which is the part that means
	...
	def __call__(self, state: Particles) -> Array:
		...
		weight_gradient = spiky_gradient_kernel(displacement, distance, self.support_radius)

		density = self.mass * jnp.sum(weight, axis=-1)

		# Each neighbor is weighted by the volume *it* stands for, not by the volume of the
		# particle doing the looking. The two agree while the cloud is even and part ways
		# once it is not, which is the whole of what a growing system does.
		volume = self.mass / jnp.maximum(density, 1e-8)

		average = jnp.einsum("...ij,...j,...jc->...ic", weight, volume, feature)

		# The difference form: what the neighbors' states are *relative to this one*. Taking
		# the states directly would leave a constant field with a non-zero gradient wherever
		# the cloud is uneven, since the kernel gradients would no longer cancel.
