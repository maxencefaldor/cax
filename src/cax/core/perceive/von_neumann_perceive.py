...
class VonNeumannPerceive(NeighborhoodPerceive):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            reduce_fn: Optional reduction function applied over the neighbor axis. If
                None, neighbors are concatenated along the channel axis. If provided, it
                is called as `reduce_fn(stacked_neighbors, axis=0)` and the result is
                concatenated with the center (if `include_center` is True).
    ...
    def _get_shifts(self) -> list[tuple[int, ...]]:
        ...
        Returns:
            List of shift tuples with Manhattan distance <= radius, excluding the
            origin.
        ...
        return [
            ...
            for shift in product(
                range(-self.radius, self.radius + 1), repeat=self.num_spatial_dims
            )
