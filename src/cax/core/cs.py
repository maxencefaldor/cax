...
evolution with JAX/Flax scanning utilities and can return the full trajectory of states.
...
class ComplexSystem[State, Input](nnx.Module):
	...
	@nnx.jit(static_argnames=("num_steps", "input_in_axis", "trajectory"))
	def __call__(
		self,
		state: State,
		...
		*,
		...
		When `trajectory` is True, the per-step states are returned as the scan's stacked
		outputs alongside the final state. The trajectory holds the state *after* each step —
		its first element is the state after one step, its last equals the final state, and
		...
		Note that `num_steps`, `input_in_axis`, and `trajectory` are static: each distinct
		combination compiles once, so sweeps over horizons should batch their step counts.
		...
		Args:
			...
			trajectory: Whether to also return the stacked per-step states.

		Returns:
			Final state after `num_steps` applications of `_step`, or a
				`(final_state, states)` tuple where `states` stacks the per-step states
				along a new leading axis of size `num_steps`.

		"""

		def step_fn(cs: ComplexSystem, state: State, input: Input | None) -> tuple[State, Any]:
			next_state = cs._step(state, input)
			return next_state, (next_state if trajectory else None)
		...
		return (state, states) if trajectory else state
