...
class MLPUpdate(Update[Array, Array, Array]):
    ...
    Maps a perception (and optional input) to the next state using pointwise
    convolutional layers (kernel size 1) applied independently at each spatial position.
    ...
    def __call__(
        ...
        If input is provided, it is concatenated to the perception along the channel
        axis before being passed through the layers.
