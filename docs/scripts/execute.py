...
def notebook_digest(path: Path, library: str) -> str:
	...
	source = "".join("".join(cell["source"]) for cell in cells if cell["cell_type"] == "code")
	return hashlib.sha256(f"{library}{source}".encode()).hexdigest()


def execute(path: Path, device: str | None) -> tuple[bool, float, str]:
	"""Execute one notebook into the build directory, returning (ok, seconds, message)."""
...
def main() -> None:
	...
	parser.add_argument("names", nargs="*", help="notebook stems; default is all stale ones")
	...
	parser.add_argument("--device", default=None, help="CUDA_VISIBLE_DEVICES for the run")
	...
	print(f"{len(notebooks)} notebooks: {len(stale)} to execute, {fresh} already current\n")
