...
if log.tell() == 0:
    log.write("epoch,soup,brotli_bytes,h0,hoe,mean_steps,species,largest,wall_s\n")
...
while epoch < a.max_epochs:
    for _ in range(a.log_every // a.frame_every):
        soup, mean_steps = chunk(cs, soup, a.frame_every)
        epoch += a.frame_every
        save_frames(soup, epoch)
    ...
    for i in range(R):
        ...
        log.write(
            f"{epoch},{i},{int(bpb * s.size / 8)},{h0:.4f},{hoe:.4f},{float(mean_steps[i]):.1f},{num_species},{largest:.4f},{time.time() - t0:.1f}\n"
        )
