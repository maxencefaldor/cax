...
if a.npz:
    ...
    raw = np.fromfile(a.file, dtype=np.uint8)
...
for i in range(0, len(soup), 2048):
    ...
    scores.append(
        np.asarray(
            replication_score(
                batch, sample_partners(jax.random.key(i), len(batch)), table
            )
        )
    )
scores = np.concatenate(scores)
print(
    "score histogram (score: count):",
    {int(k): int(v) for k, v in zip(*np.unique(scores, return_counts=True))},
)
print(
    f"replicators (score >= {a.threshold}): {(scores >= a.threshold).sum()}  (score >= 48): {(scores >= 48).sum()}"
)
