...
ap.add_argument("--controls", default="matched,cyclic,flip")
ap.add_argument("--heads", default="two,swap")
ap.add_argument("--dists", default="uniform,cust")
...
def sample(rng, n, table, dist):
    ...
    instr = np.array(
        [b for b in range(256) if bool(is_instruction(table[b]))], dtype=np.uint8
    )
    other = np.array(
        [b for b in range(256) if not bool(is_instruction(table[b]))], dtype=np.uint8
    )
    coin = rng.random((n, 64)) < 0.5
    return np.where(
        coin, rng.choice(instr, (n, 64)), rng.choice(other, (n, 64))
    ).astype(np.uint8)
            ...
            for dist in a.dists.split(","):
                ...
                for i in range(0, a.num, a.batch):
                    ...
                    partners = sample_partners(
                        jax.random.key(a.seed * 1000 + i), len(progs)
                    )
                    scores.append(
                        np.asarray(
                            replication_score(
                                jnp.asarray(progs), partners, table, control=control
                            )
                        )
                    )
                    # does the program write anything when run against a random partner?
                    pair = np.concatenate([progs, np.asarray(partners[:, 0])], axis=1)
                    out, st, _, _ = run(jnp.asarray(pair), table, control=control)
                    writes.append(np.any(np.asarray(out) != pair, axis=1))
                    steps_all.append(np.asarray(st))
                ...
                row = [
                    ...
                    dist,
                    len(s),
                    round(float(wr.mean()), 4),
                    round(float(st.mean()), 1),
                    int((s >= 8).sum()),
                    int((s >= 16).sum()),
                    int((s >= 32).sum()),
                    int((s >= 48).sum()),
                    ...
                    " ".join(f"{k}:{v}" for k, v in enumerate(hist) if v and k > 0),
                ]
                w.writerow(row)
                f.flush()
                print(row[:13], flush=True)
