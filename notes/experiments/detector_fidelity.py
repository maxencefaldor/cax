...
m = rng.random(progs.shape)
progs = np.where(m < 0.4, rng.choice(ops, progs.shape), progs).astype(np.uint8)
...
for heads in (False, True):
    ...
    mine = np.asarray(
        replication_score(
            jnp.asarray(progs), jnp.asarray(noise), table, heads_from_tape=heads
        )
    )
    print(
        f"heads={heads}: replicator scores ref={ref[:4].tolist()} mine={mine[:4].tolist()}; random progs: mismatches={int((ref != mine).sum())}/{len(progs)}, max ref score among random={int(ref[4:].max())}"
    )
