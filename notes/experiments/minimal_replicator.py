...
ap.add_argument("--with_zero", action="store_true")
ap.add_argument(
    "--sample", type=int, default=0, help="random sample size instead of exhaustive"
)
ap.add_argument(
    "--threshold",
    type=int,
    default=4,
    help="score at or above which a program is reported",
)
ap.add_argument("--batch", type=int, default=4096)
ap.add_argument("--seed", type=int, default=0)
a = ap.parse_args()
table = opcode_table_from_string() if a.dialect == "two" else opcode_table_swap_heads()
alphabet = [b for b in range(256) if bool(is_instruction(table[b]))] + (
    [0] if a.with_zero else []
)
...
print(
    f"{len(cands)} candidates of length {a.length} over {len(alphabet)} symbols",
    flush=True,
)
t0 = time.time()
found = 0
with open(a.out, "a") as f:
    ...
    for i in range(0, len(cands), a.batch):
        c = cands[i : i + a.batch]
        progs = np.zeros((len(c), 64), dtype=np.uint8)
        progs[:, : a.length] = c
        ...
        for j in np.nonzero(scores >= a.threshold)[0]:
            w.writerow(
                [
                    ...
                    a.length,
                    int(scores[j]),
                    unparse(progs[j][: a.length], opcode_table=table),
                ]
            )
            found += 1
        f.flush()
        if (i // a.batch) % 10 == 0:
            print(
                f"{i + len(c)}/{len(cands)} scored, {found} at score >= {a.threshold}, {time.time() - t0:.0f}s",
                flush=True,
            )
