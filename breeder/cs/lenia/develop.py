...
a CAX `Lenia` complex system directly, the system is simulated from the genotype's initial
state, and the observable outcome — centered rendered frames and toroidal metric time
series — forms the phenotype.
"""
...
def develop(genotype: Genotype, config: LeniaConfig, *, center: bool = True) -> Phenotype:
	"""Develop a genotype into its phenotype.

	Args:
		genotype: Genotype to develop.
		config: Lenia configuration.
		center: Whether frames are centered on the pattern. Descriptors want the centered
...
def observe(cs: Lenia, state_init: Array, config: LeniaConfig, *, center: bool = True) -> Phenotype:
	...
	metrics |= motion_series(metrics["center_of_mass"], world_size=world_size, T=config.T)
