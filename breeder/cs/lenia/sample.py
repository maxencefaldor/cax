...
def soliton(key: Array, config: LeniaConfig) -> Genotype:
    ...
    states = jnp.stack(
        [_pattern_state(name, config) for name in config.sample.pattern_names]
    )
...
def soliton_full(key: Array, config: LeniaConfig) -> Genotype:
    ...
    for name in config.sample.pattern_names:
        ...
        if rule_params.kernel_params.beta.shape[-1] != config.kernel_rank:
            raise ValueError(
                f"Pattern {name!r} does not match kernel_rank {config.kernel_rank}."
            )
        # Catalogued weights are raw growth heights; normalize onto the simplex
        rule_params = replace(
            rule_params, weight=rule_params.weight / jnp.sum(rule_params.weight)
        )
        genotypes.append(
            Genotype(rule_params=rule_params, state_init=_pattern_state(name, config))
        )
...
def channel_topology(config: LeniaConfig) -> tuple[Array, Array]:
    ...
    cross_pairs = [
        (i, j)
        for i in range(config.channel_size)
        for j in range(config.channel_size)
        if i != j
    ]
    ...
    cross_sources = jnp.repeat(
        jnp.array([i for i, _ in cross_pairs], dtype=jnp.int32),
        config.num_cross_kernels,
    )
    cross_targets = jnp.repeat(
        jnp.array([j for _, j in cross_pairs], dtype=jnp.int32),
        config.num_cross_kernels,
    )
...
def _sample_kernel_params(key: Array, config: LeniaConfig) -> LeniaKernelParams:
    ...
    r = jax.random.uniform(
        key_r, minval=config.kernel_r_range[0], maxval=config.kernel_r_range[1]
    )
...
def _place(stamp: Array, spatial_dims: tuple[int, ...]) -> Array:
    ...
    starts = tuple(
        (dim - size) // 2
        for dim, size in zip(spatial_dims, stamp.shape[:-1], strict=True)
    )
