...
class FlowLeniaConfig(LeniaConfig):
	...
	- **Mass is conserved exactly**, so the initial state fixes it forever and `min_mass`
	...
	Attributes:
		...
		theta_A: Threshold of the flow activation (the paper's symbol); the official code
	...
	# The catalogued Lenia solitons are not Flow Lenia solitons: the paper's random patch
