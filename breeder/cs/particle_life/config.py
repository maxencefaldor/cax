...
class MutateConfig(BaseModel):
    ...
    Attributes:
        ...
        position_std: Mutation standard deviation of particle positions, as a fraction
        of
...
class ParticleLifeConfig(BaseModel):
    ...
    Particles of a few classes attract and repel each other by a distance-dependent
    force: always repelling closer than `beta · r_max`, then following the attraction
    matrix out to `r_max`. The evolved genotype is the attraction matrix, the force's
    crossover `beta`, and the initial arrangement.

    What is *not* evolved is deliberate. `r_max` sets the length unit that every metric
    is expressed in, so evolving it would make the population's measurements
    incomparable — the same reason Lenia freezes `R`. `dt`, `force_factor` and
    `velocity_half_life` are integration and time-scale parameters, phenotype-neutral in
    the continuum limit.

    Attributes:
        name: Discriminator tag of the complex system.
        num_particles: Number of particles. Pairwise forces make the cost quadratic in
        it.
        ...
        num_steps: Number of simulation steps per development. The blob relaxes into its
            settled structure by roughly a thousand steps; measuring earlier measures
            the transient rather than the creature.
        ...
        particle_radius: Rendered radius of a particle, as a fraction of the torus. At
            ...
            near-invisible specks; 0.016 gives it a legible body, for the encoder as
            much as for the eye.
        min_concentration: Minimum localization at every step for a phenotype to be
        valid.
