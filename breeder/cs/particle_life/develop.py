...
def develop(genotype: Genotype, config: ParticleLifeConfig, *, center: bool = True) -> Phenotype:
	...
	metrics |= motion_series(metrics["center_of_mass"], world_size=world_size, T=1.0 / config.dt)
...
def valid(phenotype: Phenotype, config: ParticleLifeConfig) -> Array:
	...
	"""
	concentrated = jnp.all(phenotype.series["concentration"] >= config.min_concentration)
...
def _metrics(state: ParticleLifeState, config: ParticleLifeConfig) -> dict[str, Array]:
	...
	polarization = jnp.linalg.norm(jnp.mean(jnp.where(speed > 0, velocity / speed, 0.0), axis=0))
	deviation = velocity - jnp.mean(velocity, axis=0)
	thermal_velocity = jnp.sqrt(jnp.mean(jnp.sum(jnp.square(deviation), axis=-1))) / config.r_max
...
def _circular_mean(position: Array) -> tuple[Array, Array]:
	...
	cos_mean, sin_mean = jnp.mean(jnp.cos(angle), axis=0), jnp.mean(jnp.sin(angle), axis=0)
...
def _toroidal_offset(position: Array, origin: Array) -> Array:
	"""Return the shortest displacement from `origin` to `position` on the unit torus."""
	offset = position - origin
	return offset - jnp.round(offset)


def _center_state(state: ParticleLifeState) -> ParticleLifeState:
	"""Roll the arrangement so its toroidal center of mass sits at the center of the world."""
