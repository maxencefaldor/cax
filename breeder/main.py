...
One experiment is one config. All fields are settable from the command line, e.g.:
    ...
    python -m breeder.main --complex-system.sample.strategy soliton \
        --qd.num-generations 1024
...
NullableEncoderConfig = Annotated[
    EncoderConfig,
    BeforeValidator(lambda value: {"name": "none"} if value is None else value),
]
...
class Config(BaseModel):
    ...
    def check_series(self) -> Self:
        ...
        if self.encoder.window > cs.num_frames:
            raise ValueError(
                f"encoder.window {self.encoder.window} exceeds the "
                f"{cs.num_frames} frames {self.complex_system.name} renders"
            )
        ...
        references = (
            ...
            *(
                ("descriptor", s, self.descriptor.window)
                for s, _ in self.descriptor.series
            ),
        )
        for consumer, series, window in references:
            ...
            if series == self.encoder.series and window > self.encoder.window:
                raise ValueError(
                    f"{consumer}.window {window} exceeds the encoder window "
                    f"{self.encoder.window}"
                )
...
class CliConfig(Config, BaseSettings):
    ...
    model_config = SettingsConfigDict(
        cli_parse_args=True, cli_kebab_case=True, frozen=True
    )
...
def main(config: Config) -> None:
    """Run the experiment."""
    run_dir = (
        Path(config.output_dir) / f"{config.name}_{datetime.now():%Y-%m-%d_%H%M%S}"
    )
    ...
    # Mesh over all visible devices; evaluation is sharded over the child_individuals
    # axis
    mesh = jax.make_mesh(
    ...
    def log_row(metrics) -> None:
        """Host callback for one generation's metrics.

        Appends them to `log.csv` and prints them periodically.
        """
        ...
        if row["generation"] % config.log_interval == 0:
            evaluations = (
                config.qd.num_init + row["generation"] * config.qd.num_children
            )
            print(
    ...
    def evaluate_one(genotype, encoder_fn):
        ...
        phenotype = replace(
            phenotype, series=phenotype.series | encoder_fn.encode(observation)
        )
        fitness = fitness_fn(phenotype)
        descriptor = descriptor_fn(phenotype)
        invalid = (
            ~cs.valid_fn(phenotype)
            | jnp.isnan(fitness)
            | jnp.any(jnp.isnan(descriptor))
        )
    ...
    def encode_population(observations, encoder_fn):
        ...
        return jax.lax.map(
            encode_one, observations, batch_size=config.qd.minibatch_size
        )
    ...
    def run_block(key, state, generation, encoder_fn):
        ...
        def scan_step(carry, key):
            ...
            child_individuals = jax.lax.with_sharding_constraint(
                dns.ask(key, state), data_sharding
            )
            fitness, descriptor, observations = evaluate(child_individuals, encoder_fn)
            state = dns.tell(
                state, child_individuals, fitness, descriptor, observations
            )
    ...
    if config.bootstrap:
        encoder_fn = jax.device_put(
            encoder_fn.refit(key_fit, observations, valid=fitness != -jnp.inf),
            replicated_sharding,
        )
        if config.reencode:
    ...
    for block_start in range(0, config.qd.num_generations, config.train_interval):
        ...
        if generation < config.qd.num_generations:
            ...
            encoder_fn = jax.device_put(
                encoder_fn.refit(
                    subkey, state.observations, valid=state.fitness != -jnp.inf
                ),
                replicated_sharding,
            )
            if config.reencode:
                state = replace(
                    state, descriptor=encode_population(state.observations, encoder_fn)
                )
...
if __name__ == "__main__":
    cli = CliConfig()
    if cli.config:
        # Flags given alongside --config refine the file, so a variation that changes
        # one field (a seed, a name) is a command line rather than another config file
        overrides = {
            name: getattr(cli, name) for name in cli.model_fields_set - {"config"}
        }
