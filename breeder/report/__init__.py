...
def _progress(run_dir: Path, config) -> dict[str, list[float]]:
    ...
    with (run_dir / "log.csv").open(newline="") as file:
...
def _labels(config) -> tuple[str, str]:
    ...
    if getattr(config.fitness, "name", None) == "homeostasis":
        fitness = (
            f"homeostasis({config.fitness.series}) · window {config.fitness.window}"
        )
    else:
        sign = "-" if config.fitness.sign == -1 else ""
        channel = (
            f"[{config.fitness.channel}]" if config.fitness.channel is not None else ""
        )
        fitness = (
    ...
    labels = (
        {encoder.series: f"{encoder.series} ({encoder.name})"} if encoder.series else {}
    )
    descriptor = (
        "mean "
...
def write_report(
    state: DNSState, run_dir: Path, config, *, minibatch_size: int
) -> None:
    ...
    features = diversity.reference_features(
        reference_fn, last_observations, unit=cs.unit
    )
    ...
    report = {
        "summary": {
            ...
            "vendi_cos": float(
                diversity.vendi_score(features, valid, correlation=True)
            ),
        },
        ...
        "population": [
            {
                ...
                "metrics": {
                    name: float(values[rank]) for name, values in series_values.items()
                },
