...
population as diverse. Measured against the eye's verdict on the roll/no-roll pair,
every VGG measure agreed on both seeds and the random ones were a coin flip; see
...
def center_crop(frames: Array, size: int) -> Array:
    ...
    Frames must already be centered on the pattern. Cropping to a fixed *physical*
    window (`REFERENCE_CROP_RADII` kernel radii) before embedding makes the reference
    space comparable across world sizes: without it, the same creature in a larger world
    covers a smaller fraction of the frame and all distances shrink.
...
class ReferenceEncoder(nnx.Module):
    ...
    VGG16's ImageNet feature stack up to `REFERENCE_LAYER`, spatially averaged. Frames
    are resized to `REFERENCE_SPATIAL_DIMS` first: the crop preceding this embedding is
    a fixed *physical* window, so its pixel size varies with world resolution and with
    the system, and the resize is what makes one reference space serve all of them.
    """
    ...
    def __call__(self, frames: Array) -> Array:
        ...
        x = jax.image.resize(
            x, (x.shape[0], *REFERENCE_SPATIAL_DIMS, x.shape[-1]), method="linear"
        )
...
def vendi_score(features: Array, valid: Array, *, correlation: bool = False) -> Array:
    ...
    features are non-negative, so plain cosine similarities crowd near 1 and compress
    the score toward 1. Not rescaling each individual to unit length matters just as
    much: a correlation kernel discards how far an individual sits from the mean, and in
    a collapsed population the residuals are near-pure noise, which in high dimension is
    near-orthogonal — so collapse scored close to its maximum. See
    ...
    if correlation:
        x = jnp.where(
            valid[:, None], x / (jnp.linalg.norm(x, axis=-1, keepdims=True) + 1e-8), 0.0
        )
    else:
...
def reference_features(
    reference_fn: nnx.Module, frames: Array, *, unit: float
) -> Array:
    ...
    return jnp.concatenate(
        [reference_fn(frames[i : i + REFERENCE_BATCH_SIZE]) for i in batches]
    )
