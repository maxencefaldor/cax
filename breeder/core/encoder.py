...
D4 = tuple(
	partial(lambda frame, k, flip: jnp.rot90(frame[::-1] if flip else frame, k), k=k, flip=flip)
...
def augment_batch(key: Array, batch: Array, augment: tuple[Augmentation, ...]) -> Array:
	"""Randomly transform each (H, W, C) sample of a batch by the named augmentations."""
...
class Encoder(nnx.Module):
	...
	def encode(self, frames: Array) -> dict[str, Array]:
		...
		if self.invariant:
			...
			features = jnp.mean(
				jnp.stack([self.embed(jax.vmap(transform)(frames)) for transform in D4]), axis=0
			)
		else:
...
class CNNEncoder(Encoder):
	...
	pass afterwards is pure JAX and fully jittable. Requires `torch` and `torchvision` at
	construction only (CPU builds suffice — they are just weight readers).
	"""
	...
	def __init__(self, *, layer: int, window: int, invariant: bool, rngs: nnx.Rngs):
		...
		Args:
			...
			window: Number of final frames encoded, excluding the developmental transient.
...
class CNNEncoderConfig(BaseModel):
	...
	def build(self, key: Array, cs: ComplexSystem) -> Encoder:
		...
		return CNNEncoder(
			layer=self.layer, window=self.window, invariant=self.invariant, rngs=nnx.Rngs(key)
		)
...
class VAEEncoder(Encoder):
	...
	the VAE from scratch and trains it on the population's observations with the module's
	own training recipe.
	"""
	...
	def __init__(
		...
		*,
		...
		Args:
			...
			features: Feature sizes of the VAE encoder, starting with the input channels.
	...
	def refit(self, key: Array, observations: Array, *, valid: Array) -> VAEEncoder:
		...
		Args:
			...
			valid: Boolean array with shape `(N,)`; invalid individuals are never sampled.
...
class VAEEncoderConfig(BaseModel):
	"""Config of `VAEEncoder` (learned latent features, AURORA-style, the `latent` series)."""
...
def fit_scan(
	...
	*,
	...
	p = jnp.where(jnp.any(valid), valid / jnp.maximum(jnp.sum(valid), 1), 1.0 / num_individuals)
	...
	def scan_step(
		...
		individual_idx = jax.random.choice(key_individual, num_individuals, (batch_size,), p=p)
