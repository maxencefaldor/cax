...
def interior(position: jax.Array) -> jax.Array:
    """Mask particles away from the wrap, where a non-periodic comparison differs."""
    return jnp.all((position > 0.25) & (position < 0.75), axis=-1)


def test_poly6_vanishes_outside_the_support() -> None:
    """The kernel has compact support and reaches zero smoothly, not by a step."""
...
def test_spiky_gradient_points_inward_and_survives_the_origin() -> None:
    ...
    Poly6's own gradient vanishes as two particles approach, leaving the closest
    neighbors contributing nothing to a gradient estimate. Spiky's does not, which is
    why it is the one used wherever a direction is wanted.
    """
...
def test_gradient_recovers_a_linear_field(cloud: Particles) -> None:
    ...
    particles = Particles(
        position=cloud.position, state=(cloud.position @ slope)[:, None]
    )

    perception = SPHPerceive(
        support_radius=SUPPORT_RADIUS, log_normalize=False, period=1.0
    )(particles)
    gradient = perception[:, 2:4][interior(cloud.position)]

    relative_error = jnp.linalg.norm(
        jnp.mean(gradient, axis=0) - slope
    ) / jnp.linalg.norm(slope)
...
def test_gradient_of_a_constant_field_vanishes(cloud: Particles) -> None:
    ...
    gradient wherever the cloud is uneven, because the kernel gradients no longer
    cancel.
    """
...
def test_density_gradient_vanishes_on_a_uniform_cloud(cloud: Particles) -> None:
    """No direction is denser than another, so the cue that says so should be quiet."""
    perception = SPHPerceive(
        support_radius=SUPPORT_RADIUS, log_normalize=False, period=1.0
    )(cloud)
...
def test_perception_differentiates_where_nothing_is_happening(cloud: Particles) -> None:
    ...
    neighborhood where every state is zero, so this is the common case rather than an
    edge one, and getting it wrong turns the whole model to `nan` on the first step.
    """
...
def test_log_normalize_keeps_direction_and_compresses_length(cloud: Particles) -> None:
    ...
    A kernel gradient carries the support radius to a negative power, so it arrives
    orders of magnitude larger than the states beside it. The direction is the part that
    means something; the magnitude only has to be on a comparable scale.
    """
    slope = jnp.array([1.3, -0.7])
    particles = Particles(
        position=cloud.position, state=(cloud.position @ slope)[:, None]
    ...
    raw = SPHPerceive(support_radius=SUPPORT_RADIUS, log_normalize=False, period=1.0)(
        particles
    )
...
def both_ways(num_particles: int, *, fused: bool) -> SPHPerceive:
    ...
    return SPHPerceive(
        support_radius=0.2, mass=1.0 / num_particles, period=1.0, fused=fused
    )


@gpu_only
def test_fused_matches_the_array_path() -> None:
    """The kernels are only worth having if they compute what the array version does.

    Everything downstream rests on this: a perception that is close but not equal trains
...
def test_fused_gradients_match_the_array_path() -> None:
    ...
    A wrong derivative does not raise; it trains something else. This is the check that
    the hand-derived backward is the derivative of the forward beside it.
    """
    num_particles, channel_size = 1024, 16
    particles = Particles(
        position=jax.random.uniform(jax.random.key(1), (num_particles, 2)),
...
def test_fused_maps_over_a_batch() -> None:
    ...
    batch = Particles(
        ...
        state=jax.random.normal(jax.random.key(2), (3, num_particles, channel_size))
        * 0.3,
    )
...
def test_fused_matches_at_any_cloud_size(num_particles: int) -> None:
    ...
    A count the tiles do not divide is padded out and the extra entries are masked away,
    so sizes that are awkward for the kernel must still give the array version's answer
