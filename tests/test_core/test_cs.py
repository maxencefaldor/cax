...
def test_returned_states_match_final_state(elementary: Elementary) -> None:
    ...
    num_steps = 8
    state_final, states = elementary(
        state_init(), num_steps=num_steps, return_states=True
    )
...
def test_driver_emits_no_warning(elementary: Elementary) -> None:
    """Test that no driver hits a deprecation path (e.g. sow outside nnx.capture)."""
...
def test_final_only_call_matches_return_states_call(elementary: Elementary) -> None:
    """Test that a final-only call matches the final state of `return_states=True`."""
    state_final = elementary(state_init(), num_steps=6)
    state_final_with_states, _ = elementary(
        state_init(), num_steps=6, return_states=True
    )
