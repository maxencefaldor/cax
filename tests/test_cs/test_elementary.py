...
def test_rule_110_known_rows() -> None:
    ...
    # Rule 110 from ...00000100...: one step grows left (00001100), two steps
    # (00011100).
    assert jnp.array_equal(
        states[0][:, 0], jnp.array([0, 0, 0, 0, 1, 1, 0, 0], dtype=jnp.float32)
    )
    assert jnp.array_equal(
        states[1][:, 0], jnp.array([0, 0, 0, 1, 1, 1, 0, 0], dtype=jnp.float32)
    )
