...
def test_particle_lenia_jit_init() -> None:
    ...
    def init_particle_lenia() -> ParticleLenia:
        ...
        growth_params = ParticleLeniaGrowthParams(
            mean=jnp.array([0.5]), std=jnp.array([0.1])
        )
...
def test_particle_lenia_fields_match_reference() -> None:
    ...
    growth_params = ParticleLeniaGrowthParams(
        mean=jnp.array(mu_g), std=jnp.array(sigma_g)
    )
    rule_params = ParticleLeniaRuleParams(
    ...
    state = ParticleLeniaState(
        position=jax.random.uniform(key, (12, 2), minval=-4.0, maxval=4.0)
    )
...
def test_particle_lenia_render_modes_differ() -> None:
    ...
    key = jax.random.key(0)
    state = ParticleLeniaState(
        position=jax.random.uniform(key, (16, 2), minval=-6.0, maxval=6.0)
    )
