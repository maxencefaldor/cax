...
These pin down the differentiability facts that `examples/55_lenia_grad.ipynb` relies
on: gradients flow through entire rollouts, normalized parameters are flat along their
scale direction, and the kernel radius is differentiable exactly when the kernel core
vanishes at its support boundary.
"""
...
def float_and_static(
    rule_params: LeniaRuleParams,
) -> tuple[LeniaRuleParams, LeniaRuleParams]:
...
def test_scan_rollout_matches_driver_bitwise() -> None:
    ...
    lenia = Lenia(
        spatial_dims=(32, 32), channel_size=1, R=5, T=10, rule_params=rule_params
    )
...
def test_gradient_flows_through_rollout() -> None:
    """Test that a rollout's gradient is finite, and nonzero for the growth params."""
    ...
    def loss_fn(params: LeniaRuleParams) -> jax.Array:
        return rollout_loss(
            jax.tree.map(
                lambda p, s: s if p is None else p,
                params,
                static,
...
def test_normalized_parameters_have_zero_scale_gradient() -> None:
    ...
    The kernel weights are divided by their sum and the kernel by its own integral, so
    the loss is scale-free in `weight` and in each kernel's `beta`. For a degree-0
    homogeneous function, Euler's theorem forces `sum_k theta_k * dL/dtheta_k = 0`.
    """
    ...
    beta_scale = jnp.sum(
        rule_params.kernel_params.beta * grads.kernel_params.beta, axis=-1
    )
...
def test_off_band_potential_collapses_the_rule_gradient() -> None:
    ...
    to a gradient some fifteen orders of magnitude below their in-band value. In a
    narrow enough band the bell underflows and the gradient is exactly zero.
    ...
    def rule_gradient_norm(num_steps: int) -> jax.Array:
        grads = jax.grad(
            lambda params: rollout_loss(merge(params), state, num_steps=num_steps)
        )(params)
        return jnp.sqrt(
    ...
    state_grads = jax.grad(lambda state: rollout_loss(rule_params, state, num_steps=1))(
        state
    )
...
def test_building_a_system_under_jit_leaves_the_rule_intact() -> None:
    ...
    def loss(state: jax.Array) -> jax.Array:
        lenia = Lenia(
            spatial_dims=(32, 32), channel_size=1, R=13, T=10, rule_params=rule_params
        )
...
def test_the_driver_composes_with_nested_transformations() -> None:
    ...
    itself a reliable regression test —
    `test_building_a_system_under_jit_leaves_the_rule_intact` covers the cause. This
    covers the pattern end to end.
    """
    from cax.cs.lenia import load_pattern
    ...
    def build() -> Lenia:
        return Lenia(
            spatial_dims=(64, 64), channel_size=1, R=13, T=10, rule_params=rule_params
        )

    @jax.jit
    def descend(state: jax.Array) -> jax.Array:
        return state - 0.01 * jax.grad(lambda s: jnp.sum(build()(s, num_steps=16)))(
            state
        )
