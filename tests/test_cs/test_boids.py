...
def test_boid_policy_isolated_boid_is_finite() -> None:
    """Test that a boid with no neighbor in range gives finite forces and gradients."""
    ...
    def loss(position: jax.Array) -> jax.Array:
        inner_policy = BoidsPolicy(
            perception_radius=0.01, noise_scale=0.0, rngs=nnx.Rngs(0)
        )
        return jnp.sum(
            inner_policy(BoidsState(position=position, velocity=velocity), 0)
        )
