...
def single_channel_rule_params(
    mean: float = 0.15, std: float = 0.015
) -> LeniaRuleParams:
...
def test_flow_lenia_conserves_mass_in_any_dimension(
...
def test_sobel_matches_analytic_gradient_in_3d() -> None:
    ...
    field = sum(
        slope * coordinate
        for slope, coordinate in zip(slopes, coordinates, strict=True)
    )
...
def test_sobel_commutes_with_translation(spatial_dims: tuple[int, ...]) -> None:
    ...
    assert jnp.array_equal(
        sobel(jnp.roll(field, shift, axis=axes)),
        jnp.roll(sobel(field), shift, axis=axes),
    )
...
def test_flow_lenia_is_isotropic_in_3d() -> None:
    ...
    for spatial_axis in range(3):
        assert jnp.allclose(
            state_final, jnp.flip(state_final, axis=spatial_axis), atol=1e-4
        )


def test_flow_lenia_theta_a_defaults_to_channel_size() -> None:
    """Test that theta_A defaults to channel_size, as in the official implementation."""
    rule_params = LeniaRuleParams(
