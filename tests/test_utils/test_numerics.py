...
def test_safe_divide_values() -> None:
    """Test that safe_divide matches division where valid and is zero elsewhere."""
...
def test_safe_divide_gradient_is_finite_at_singularity() -> None:
    ...
    def loss(denominator: jax.Array) -> jax.Array:
        return jnp.sum(
            safe_divide(
                jnp.ones_like(denominator), denominator, where=denominator > 0.0
            )
        )
...
def test_safe_norm_values() -> None:
    """Test that safe_norm matches jnp.linalg.norm on nonzero vectors, zero at zero."""
...
def test_safe_norm_gradient_is_finite_at_origin() -> None:
    ...
    grad = jax.grad(lambda vector: jnp.sum(safe_norm(vector, axis=-1)))(
        jnp.zeros((2, 3))
    )
    assert jnp.isfinite(grad).all()

    raw_grad = jax.grad(lambda vector: jnp.sum(jnp.linalg.norm(vector, axis=-1)))(
        jnp.zeros((2, 3))
    )
