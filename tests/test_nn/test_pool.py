...
def test_pool_sample(init_pool):
	...
	index, sampled_data = init_pool.sample(key, sample_size=3)
...
def test_pool_add(init_pool):
	"""Test the add method of the Pool class."""
	new_index = jnp.array([0, 2, 4])
	new_a = jnp.array([100, 200, 300])
	new_b = jnp.array([1000, 2000, 3000])

	updated_pool = init_pool.add(new_index, a=new_a, b=new_b)
