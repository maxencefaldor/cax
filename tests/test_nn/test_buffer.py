...
def test_buffer_add(buffer: Buffer) -> None:
    ...
    assert jnp.array_equal(
        new_buffer.is_full, jnp.array([True, True, False, False, False])
    )
...
def test_buffer_add_wraparound(buffer: Buffer) -> None:
    ...
    assert jnp.all(buffer.is_full)
    assert jnp.array_equal(
        buffer.data["a"], jnp.array([[6.0], [2.0], [3.0], [4.0], [5.0]])
    )
    assert jnp.array_equal(
        buffer.data["b"], jnp.array([[11, 12], [3, 4], [5, 6], [7, 8], [9, 10]])
    )
